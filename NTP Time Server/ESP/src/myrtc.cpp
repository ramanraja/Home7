...
// If power was lost and restored, rtc.lostPower() will indicate this only the first time; After a soft reset, this information is lost.
// So you need a mechanism to determine later if the time is valid.
// On the first cold boot, time will be set to LAST_COMPILE_TIME. For the next one hour the time will be invalid. Sense this and take action.
bool MyRTC::is_valid() {
    bool valid = (rtc.now() > AN_HOUR_AFTER_LAST_COMPILE_TIME);  // you can also use A_DAY_AFTER_LAST_COMPILE_TIME for a day-long indication of dirty clock
    if (!valid)
        SERIAL_PRINTLN(F("\n *** RTC time is invalid. Please call set_time() first!")); // TODO: keep blinking an LED; and do not serve the wrong time to clients!
    return (valid);
}
...
unsigned long MyRTC::get_epoch_time_1900 () {
    now = rtc.now(); // this is 1900 based
    return now.unixtime();
}

// This is a convenience method, since many clients like Tasmota expect the 1970 baseline
unsigned long MyRTC::get_epoch_time_1970 () {
    now = rtc.now();
    return now.unixtime() + OFFSET_1970;
}
...
void MyRTC::print_epoch_time () {
    SERIAL_PRINTLN (get_epoch_time_1900());
    //SERIAL_PRINTLN (get_epoch_time_1970());    
}
