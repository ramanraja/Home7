...
void setup()
{
    ...
    bool result;
    //result = W.init(); // DHCP IP
    //W.init(0, 95);  // for testing
    W.init(1, 95);  // for Mithila AP
    if (!result)
        SERIAL_PRINTLN (F("\nGoing ahead without WiFi. Will check again later."));
    R.init();  // RTC
...
// Happy NOTE: you need not call this again, when reconnect occurs.
// The UDP socket works even if it is initialized when there was no WiFi.
void init_udp_server () {
//    if (!W.is_connected()) {
//        SERIAL_PRINTLN ("\nNo WiFi; cannot initialize UDP"); // you can initialize without WiFi.
//        return;
//    }
