...
Acknowledgement: Every incoming command is echoed as an acknowledgement.  
Error: Command packet errors/ invalid commands are indicated by the packet: "E \0"
...
// This checks MQTT connection status
// TODO: check ethernet connection status also and retry initializing Ethernet card
void check_network() {  
    ...
    switch (cmd[0]) {   
      ...
      case 'C':
        if (cmd[2]=='1')
            ...
            mode = MODE_HALF;
        break;    
      case 'D':  // cmd should still have '1' or '0', but it is ignored
        mode = MODE_HALF;
        break;
...
void send_initial_status () {
    ...
    read_sensors();      // read sensors and compose the payload
    disable_sensors();
    status_msg[0]= 'R';  // overwrite the first byte, R indicating 'rebooted'
    send_status();
...
void send_null_status() { // sensors remain switched off, only heart beat packets are sent
    status_msg[0]= 'N';
    status_msg[1]= ' ';
